...
const ShopkeeperRegistration = () => {
  const [formData, setFormData] = useState({
    ...
    email: '',
    phno: '',
    address: '', // Add address field in state
    licenseno: '',
    licenseImage: null,
    password: '',
    confirmPassword: '',
...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!shopnameRegex.test(formData.shopname)) {
      alert('Shop Name should contain only letters, numbers, and spaces.');
      return;
    }
    ...
    if (!licenseRegex.test(formData.licenseno)) {
      alert('License number should be alphanumeric.');
      return;
    }
    ...
    // Create a new FormData object to send the form data as multipart data
    ...
    try {
      const res = await axios.post('http://localhost:5000/shopkeeper/register', data);
  ...
  return (
    <Box sx={{ backgroundColor: '#f5f5f5', marginTop: '-8px', marginBottom: '-8px', marginRight: '-8px', marginLeft: '-8px' }}>
      <Navbar />
          ...
          <Paper elevation={6} sx={{ p: 4, borderRadius: 4, marginTop: 9 }}>
            ...
            <form onSubmit={handleSubmit} encType="multipart/form-data">
              <Grid container spacing={3}>
                {[{ label: 'Name', name: 'name' }, { label: 'Shop Name', name: 'shopname' }, { label: 'Email', name: 'email', type: 'email' }, { label: 'Phone Number', name: 'phno' }, { label: 'License Number', name: 'licenseno' }].map((field, idx) => (
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    required
                    multiline
                    rows={4}  // Defines the height of the textarea
                    sx={{ width: '100%' }}
                  />
                </Grid>

                <Grid item xs={12}>
                  <Typography variant="subtitle1" mb={1}>Upload License Image</Typography>
